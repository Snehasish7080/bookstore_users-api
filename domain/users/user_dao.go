...
import (
	...
	"github.com/Snehasish7080/bookstore_users-api/datasources/mysql/users_db"
...
func (user *User) Save() *errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to get last insert id after creating new user", saveErr)
		return errors.NewInternalServerError("database error")
	}
	user.Id = userId
	return nil
...
func (user *User) FindByStatus(status string) ([]User, *errors.RestErr) {
	...
	if err != nil {
		logger.Error("error when trying to prepare Search user by status", err)
		return nil, errors.NewInternalServerError("database error")
	}
	defer rows.Close()
